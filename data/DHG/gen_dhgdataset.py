...
root_dataset_path = 'E:/DataSets/DHG2016' # 数据集根目录
...
for i in range(Samples_sum): # 遍历每一个样本
    ...
    for idx in range(num_subject):
        if idx == int(idx_subject) - 1:
            if not os.path.exists(root_dataset_path + "/DHG14-28_sample_json/" + str(idx+1)+'/val/'):
                os.makedirs(root_dataset_path + "/DHG14-28_sample_json/" + str(idx+1)+'/val/')
            with open(root_dataset_path + "/DHG14-28_sample_json/" + str(idx+1)+'/val/' \
                      + file_name + ".json", 'w') as f:
                json.dump(data_json, f)
            val_data_dict[idx].append(tmp_data_dict)

        else:
            if not os.path.exists(root_dataset_path + "/DHG14-28_sample_json/" + str(idx+1)+'/train/'):
                os.makedirs(root_dataset_path + "/DHG14-28_sample_json/" + str(idx+1)+'/train/')
            with open(root_dataset_path + "/DHG14-28_sample_json/" + str(idx+1) + '/train/' \
                      + file_name + ".json", 'w') as f:
                json.dump(data_json, f)
...
for idx in range(num_subject):
    if not os.path.exists(root_dataset_path + "/DHG14-28_sample_json/" + str(idx + 1) + "/") :
        os.makedirs(root_dataset_path + "/DHG14-28_sample_json/" + str(idx + 1) + "/")
    with open(root_dataset_path + "/DHG14-28_sample_json/" + str(idx + 1) + "/" + str(idx + 1) + "train_samples.json", 'w') as t1:
        json.dump(train_data_dict[idx], t1)

    if not os.path.exists(root_dataset_path + "/DHG14-28_sample_json/" + str(idx + 1) + "/") :
        os.makedirs(root_dataset_path + "/DHG14-28_sample_json/" + str(idx + 1) + "/")
    with open(root_dataset_path + "/DHG14-28_sample_json/" + str(idx + 1) + "/" + str(idx + 1) + "val_samples.json", 'w') as t2:
