...
root_database_path = 'E:/DataSets/SHREC2017' # 数据集根目录
# root_database_path = './shrec17_dataset/HandGestureDataset_SHREC2017' # 数据集根目录
train_txt_path = 'E:/DataSets/SHREC2017/train_gestures.txt' # 训练train_gestures.txt 的路径
...
for i in range(Samples_sum): # 遍历每一个样本
    ...
    if not os.path.exists(root_database_path + "/shrec17_jsons/train_jsons/"):
        os.makedirs(root_database_path + "/shrec17_jsons/train_jsons/")
    with open(root_database_path + "/shrec17_jsons/train_jsons/" + file_name + ".json", 'w') as f:
...
if not os.path.exists(root_database_path+"/shrec17_jsons/"):
    os.makedirs(root_database_path+"/shrec17_jsons/")
with open(root_database_path+"/shrec17_jsons/" + "train_samples.json", 'w') as t: # 将所有样本的信息保存为一个json格式文件
