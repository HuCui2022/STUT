...
def get_parser():
    ...
    parser.add_argument('--work_dir', default='./work_dir/ncla/test', help='the work folder for storing results')
    # parser.add_argument('--config', default='./config/st-gcn.yaml', help='path to the configuration file')
    parser.add_argument('--config', default='./config/st-gcn-ucla.yaml', help='path to the configuration file')
    ...
    parser.add_argument('--pin_memory', default=True, type=bool, help='the dataloader pin_memory')
    # model
    parser.add_argument('--model', default='model.st_gcn.Model', help='the model will be used')
    parser.add_argument('--label_smoothing', default=0.0, help='the model will be used')
...
class data_prefetcher():
    ...
    def preload(self):
        try:
            self.next_data, self.next_label, self.next_sample = next(self.loader)  # 从loader 读取下一组数据。
            # self.next_input, self.next_target = next(self.loader)
        except StopIteration:
            # self.next_input = None
            # self.next_target = None
            self.next_data, self.next_label, self.next_sample = None, None, None
            return
        with torch.cuda.stream(self.stream):
            self.next_data = self.next_data.float().cuda(non_blocking=True)
            self.next_label = self.next_label.long().cuda(non_blocking=True)

    def next(self):
        torch.cuda.current_stream().wait_stream(self.stream)
        features = self.next_data
        targets = self.next_label
...
class Processor():
    ...
    def load_model(self):
        ...
        # print(self.model)
        self.loss = nn.CrossEntropyLoss(label_smoothing=self.arg.label_smoothing).cuda(output_device)
    ...
    def train(self, epoch, save_model=False):
        ...
        if epoch + 1 == self.arg.num_epoch:  # todo : add to save the best weight , not to save the final weight
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        for ln in loader_name:
            ...
            if self.arg.run_mode == 'train':
                self.val_writer.add_scalar('top1', top1.avg, self.global_step)
                self.val_writer.add_scalar('loss', losses.avg, self.global_step)
                ######### add store the best weight:
                if top1.avg > self.best_acc:
                    # need the resume param, refer to the model define processing in 'load model':
                    # epoch, weight,
                    state_dict = self.model.state_dict()
                    weights = OrderedDict([[k.split('module.')[-1], v.cpu()] for k, v in state_dict.items()])
                    checkpoint = {'checkpoint': weights, 'epoch': epoch}
                    torch.save(checkpoint,
                               self.arg.work_dir + '/' + self.arg.work_dir.split('/')[-1] + '_best_checkpoint.pt')

                    # save the best score and every class scores
                    # acc for each class:
