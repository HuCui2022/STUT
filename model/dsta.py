        ...
        if use_spatial_att:
            ...
            if att_s:
                ...
                if self.num_node == 18:  # Kinetics series
                    ...
                    from graph.ntu_graph2 import AdjMatrixGraph
        ...
        if self.use_spatial_att:
            ...
            if self.att_s:
                ...
                diff = (self.diff_net(torch.einsum('nctu,uv->nctv', y, self.graph_a)).repeat(1, 1, 1,
                    ...
                    .view(N, self.num_subset, self.in_channels // self.num_subset, T, self.window_size,
                          V)  # nsctwv   计算差值。
                diff = torch.sigmoid(diff.mean(-1).mean(2).mean(2))  # nsw
                attention = attention * diff.unsqueeze(-1).repeat(1, 1, self.num_node,
                                                                  1)  # nsw -> n s w 1 -> n s wv 1; attention shape : n s u v;  不同 w 中的帧中拥有不同的权重。
            if self.glo_reg_s:
